...
def extract_words(text):
    ...
    word = list()
    word_added = True
    text_with_dot = ''.join((text,'.')) #ugly
    for character in text_with_dot:
        if character in ascii_letters:
            word_added = False
            word.append(character)
        else:
            if not word_added:
                split_text.append(''.join(word))
                word_added = True
                word = []
    return split_text
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    average = 0;
    no_sentiment_found = 1
    num_words = 0;
    words = tweet_words(tweet)
    for word in words:
        sentiment = get_word_sentiment(word)
        if has_sentiment(sentiment):
            no_sentiment_found = 0
            average += sentiment_value(sentiment)
            num_words += 1
    if no_sentiment_found:
        return make_sentiment(None)
    else:
        return make_sentiment(average/num_words)
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon) - 1):
        member = (latitude(polygon[i]) * longitude(polygon[i+1]) - latitude(polygon[i+1])
                 * longitude(polygon[i]))
        area += member
        centroid_lat += member * (latitude(polygon[i]) + latitude(polygon[i+1]))
        centroid_lon += member * (longitude(polygon[i]) + longitude(polygon[i+1]))
    area = area / 2
    if not area: 
        return (latitude(polygon[0]),longitude(polygon[0]),area)
    else:
        centroid_lat = centroid_lat/(area * 6)
        centroid_lon = centroid_lon/(area * 6)
